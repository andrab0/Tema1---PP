...
(define (enqueue x q)
  (define Q (struct-copy queue q [right (append (list x) (queue-right q))] [size-r (add1 (queue-size-r q))]))
  q
  Q)
...
(define (dequeue q)
  (match q
    [(queue left right size-l size-r)
     (if (null? left)
         (make-queue (cdr (reverse right)) '() (sub1 size-r) 0)
         (make-queue (cdr left) right (sub1 size-l) size-r))]))
