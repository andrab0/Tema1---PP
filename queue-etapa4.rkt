...
(define (enqueue x q)
  (if (>= (queue-size-l q) (add1 (queue-size-r q)))
      (struct-copy queue q [right (append (list x) (queue-right q))] [size-r (add1 (queue-size-r q))])
      (struct-copy queue q [left (rotate (queue-left q) (append (list x) (queue-right q)) null)] [right '()] [size-l (+ (queue-size-l q) (add1 (queue-size-r q)))] [size-r 0])))
  ...
  (match q
    [(queue left right size-l size-r)
     (if (>= (sub1 size-l) size-r)
         ...
         (make-queue (rotate (stream-rest left) right null) '() (+ (sub1 size-l) size-r) 0))]))
