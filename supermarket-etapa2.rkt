...
(define (empty-counter index)
  (define empty-counter (make-counter index 0 0 '()))
  empty-counter)
...
(define (add-to-counter name n-items)
  (λ (C)
    (match C
    [(counter index tt et queue)
     (if (null? queue) (make-counter index (+ tt n-items) (+ et n-items) (append queue (list (cons name n-items))))
...
(define (calcul-tt queue)
  (if (null? queue) 0
      (+ (cdr (car queue)) (calcul-tt (cdr queue))))) 

(define (remove-first-from-counter C)
  (if (= (length (counter-queue C)) 1) (struct-copy counter C [tt 0] [et 0] [queue null])
      (struct-copy counter C [tt (calcul-tt (cdr (counter-queue C)))] [et (cdr (cadr (counter-queue C)))] [queue (cdr (counter-queue C))])))    
...
(define (serve requests fast-counters slow-counters)
  (if (null? requests) (append fast-counters slow-counters)
      (match (car requests) 
        ...
        [(list 'remove-first)
         (let ([counters-list (valid-counters counter-queue (append fast-counters slow-counters))])
              (if (= (length counters-list) 0)
                  (serve (cdr requests) fast-counters slow-counters)
                  (if (<= (car (min-et counters-list)) (length fast-counters))
                      (serve (cdr requests)
                             (foldr (λ (C acc) (if (= (counter-index C) (car (min-et counters-list)))
                                                  (cons (remove-first-from-counter C) acc)
                                                  (cons C acc))) null fast-counters)
                             slow-counters)  
                     (serve (cdr requests)
                            fast-counters
                            (foldr (λ (C acc) (if (= (counter-index C) (car (min-et counters-list)))
                                                 (cons (remove-first-from-counter C) acc)
                                                 (cons C acc))) null slow-counters)))))])))
