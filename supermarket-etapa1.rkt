...
(define (empty-counter index)
  (define empty-counter (make-counter index 0 '()))
  empty-counter)
...
(define (min-tt-help counters acc)
  (cond
    ((null? counters) acc)
    ((<= (cdr acc) (counter-tt (car counters))) (min-tt-help (cdr counters) acc))
    (else
     (min-tt-help (cdr counters) (cons (counter-index (car counters)) (counter-tt (car counters)))))))

(define (min-tt counters)
  (min-tt-help counters (cons (counter-index (car counters)) (counter-tt (car counters)))))
...
(define (serve requests C1 C2 C3 C4)
  ...
  (if (null? requests)
      ...
      (match (car requests)
        ...
        [(list name n-items) (if (<= n-items ITEMS) (cond
